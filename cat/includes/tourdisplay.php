...
$query = "SELECT * FROM tourism_images WHERE t_id = '$demo' ";
            ...
            <div class="infocon" style="background-color:rgb(248, 248, 248)" >            <!--div for info container-->
                <div class=" adjust ">                         <!--full info-->
                    <div> 
                        <?php
                        $id = $_GET['id'];
                        $query = "SELECT t_name,t_add,t_rate FROM tourism WHERE t_id = '$id' ";
                        $res = mysqli_query($connection,$query);
                        while($row=mysqli_fetch_assoc($res))
                        {
                            $h_name=$row['t_name'];
                ...
                <div >               <!--description-->
                    <div >
                        <div>
                            ...
                            <?php
                             $id = $_GET['id']; 
                             $query = "SELECT t_about FROM tourism WHERE t_id = '$id'";
                             $res=mysqli_query($connection,$query);
                             while($row=mysqli_fetch_assoc($res))
                             {
                                 $h_about=$row['t_about'];
