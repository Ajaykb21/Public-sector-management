...
$query = "SELECT * FROM health_images WHERE h_id = '$demo' ";
            ...
            <div class="infocon" style="background-color:rgb(248, 248, 248)" >            <!--div for info container-->
                <div class=" adjust ">                         <!--full info-->
                    <div> 
                        <?php
                        $id = $_GET['id'];
                        $query = "SELECT h_name,h_add,h_contact FROM healthcare WHERE h_id = '$id' ";
                        $res = mysqli_query($connection,$query);
                        while($row=mysqli_fetch_assoc($res))
                        {
                            $h_name=$row['h_name'];
                            $h_addr=$row['h_add'];
                            $h_phone =$row['h_contact'];
                ...
                <div >               <!--description-->
                    <div >
                        <div>
                            ...
                            <?php
                             $id = $_GET['id']; 
                             $query = "SELECT h_about FROM healthcare WHERE h_id = '$id'";
                             $res=mysqli_query($connection,$query);
                             while($row=mysqli_fetch_assoc($res))
                             {
                                 $h_about=$row['h_about'];
                ...
                <div >                 <!-- department details-->
                    <h1 class=" gcolor adjust"> Faculty Details</h1>
                        ...
                        <div class="infocon1" style="background-color:rgb(250, 250, 250)">
                            <?php
                            $id = $_GET['id'];
                            $query="SELECT * FROM departments dp INNER JOIN doctors d  ON d.dept_id=dp.dept_id  WHERE dp.h_id='$id' GROUP BY d.doc_name ";
                            $res=mysqli_query($connection,$query) OR die('query failed');
                            while($row=mysqli_fetch_assoc($res))
                            {
                                $d_name=$row['doc_name'];
                                $dp_name=$row['dept_name'];
                                $d_specl=$row['doc_spec'];
